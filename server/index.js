...
if (isTestEnv) {
  ...
  [
    "VITE_RECURSE_CLIENT_ID",
    "VITE_RECURSE_CLIENT_SECRET",
  ...
  if (localEnvResult.error) {
    console.log(
      "No .env.local found or error loading it:",
      localEnvResult.error.message
    );
...
if (!process.env.VITE_RECURSE_CLIENT_ID) {
  ...
  console.log(
    `Client ID: ${process.env.VITE_RECURSE_CLIENT_ID.substring(0, 8)}...`
  );
}
...
app.post("/api/auth/callback", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch user profile from Recurse Center" });
    }
    ...
    try {
      console.log(
        "Attempting to save user to database with recurse_id:",
        userData.id
      );

      // First check if user with this recurse_id already exists
      const userCheck = await db.query(
        "SELECT * FROM users WHERE recurse_id = $1",
        [userData.id]
      );
...
if (process.env.NODE_ENV !== "production") {
    ...
    try {
      for (const user of users) {
        ...
        if (userCheck.rows.length === 0) {
          ...
          console.log(
            `Test user already exists: ${user.name} (ID: ${user.id})`
          );
        }
      }
  ...
  app.post("/api/test/cleanup", async (req, res) => {
    ...
    try {
      ...
      console.log(
        `Deleted ${deleteBookingsResult.rowCount} test bookings: ${
          deletedBookingIds.join(", ") || "none"
        }`
      );
      ...
      console.log(
        `Test rooms (preserved): ${roomsResult.rows
          .map((r) => `${r.id}:${r.name}`)
          .join(", ")}`
      );
...
if (process.env.NODE_ENV === "test") {
  app.use((req, res, next) => {
    console.log(
      `[TEST] ${req.method} ${req.path} - Auth token: ${
        req.cookies?.auth_token ? "Present" : "Missing"
      }`
    );
    next();
  });
